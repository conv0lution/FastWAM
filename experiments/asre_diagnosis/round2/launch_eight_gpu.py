"""Launch the pre-registered ASRE Round-2 online matrix on eight GPUs.
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Launch ASRE Round-2 keep schedules without DDP."
    )
...
def _source_manifest_sample_ids(path: Path) -> set[str]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            try:
                ...
                raise ValueError(f"Invalid source manifest record at {path}:{line_number}") from exc
            if sample_id in sample_ids:
                raise ValueError(f"Duplicate sample_id {sample_id!r} in source manifest {path}.")
            sample_ids.add(sample_id)
    if not sample_ids:
        raise ValueError(f"Source state-bank manifest is empty: {path}")
    return sample_ids
...
def _load_provenance(
    ...
    print("Validating immutable Round-2 artifact hashes...", flush=True)
    ...
    valid_sample_ids_value = payload["valid_sample_ids"]
    if not isinstance(valid_sample_ids_value, list) or not valid_sample_ids_value:
        raise ValueError("QC valid manifest valid_sample_ids must be a non-empty list.")
    valid_sample_ids = [str(value) for value in valid_sample_ids_value]
    if len(valid_sample_ids) != len(set(valid_sample_ids)):
        raise ValueError("QC valid manifest contains duplicate valid_sample_ids.")
    source_sample_ids = _source_manifest_sample_ids(source_manifest_path)
    unknown_ids = sorted(set(valid_sample_ids) - source_sample_ids)
    if unknown_ids:
        raise ValueError(
            "QC valid manifest refers to samples absent from the source manifest: "
            f"{unknown_ids[:10]}"
        )
    if payload["qc_rule"] in (None, "", {}, []):
        raise ValueError("QC valid manifest qc_rule must be non-empty.")
...
def _resolve_runtime(task_config: str, mode: str) -> RuntimeSpec:
    ...
    if num_layers != NUM_LAYERS:
        raise ValueError(
            f"ASRE Round 2 requires {NUM_LAYERS} action layers, task config exposes {num_layers}."
        )
    ...
    if observed != required:
        raise ValueError(
            "Task config does not match the pre-registered Round-2 runtime "
...
def _condition_indices(mode: str) -> tuple[int, ...]:
    return SMOKE_CONDITION_INDICES if mode == "smoke" else tuple(range(8))
    ...
    *,
    ...
    if status_path.exists():
        ...
        status_expected = {
            ...
            "physical_gpu": build_round2_conditions(NUM_LAYERS).index(condition),
        }
...
def _gpu_inventory() -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise RuntimeError(f"Cannot query the eight physical GPUs with nvidia-smi: {exc}") from exc
    ...
    missing = [index for index in range(8) if index not in by_index]
    if missing:
        raise RuntimeError(f"Round 2 requires physical GPUs 0..7; missing {missing}.")
    selected = [by_index[index] for index in range(8)]
    names = {record["name"] for record in selected}
    if len(names) != 1 or not all("A5000" in name for name in names):
        raise RuntimeError(
            "Round 2 is pre-registered for eight identical A5000 GPUs; "
...
def _validate_root_layout(
    ...
    if unexpected:
        raise DirectorySafetyError(
            f"Round-2 output root contains unexpected entries: {output_root}: {unexpected}"
        )
...
def _validate_smoke_gate(
    ...
    *,
    ...
    summary = _read_json(path, label="Round-2 smoke summary")
    expected_names = [
        build_round2_conditions(NUM_LAYERS)[index].name
...
def _condition_command(
    *,
    ...
    return [
        ...
        f"ASRE_DIAGNOSIS.protocol={ROUND2_PROTOCOL}",
...
def _main_locked(
    ...
    *,
    ...
    all_conditions = build_round2_conditions(NUM_LAYERS)
    condition_pairs = [
    ...
    try:
        ...
        interrupted = True
        launch_error = exc
        print("Interrupt received; terminating all active Round-2 children.", file=sys.stderr)
        for live in live_processes:
