...
OFFLINE_DIR="${ROUND2_ROOT}/offline"
LOG_DIR="${ROUND2_ROOT}/logs"
QC_GPU="${ROUND2_QC_GPU:-0}"
...
for required_path in "${PYTHON_BIN}" "${CHECKPOINT}" "${DATASET_STATS}" \
  "${STATE_BANK_DIR}/manifest.jsonl" "${STATE_BANK_DIR}/run_metadata.json"; do
  if [[ ! -r "${required_path}" ]]; then
    printf 'Required input is unavailable: %s\n' "${required_path}" >&2
    exit 1
  fi
done
mkdir -p "${ROUND2_ROOT}" "${OFFLINE_DIR}" "${LOG_DIR}"

printf 'Validating the Round-1 state bank with the pre-registered Round-2 QC rule.\n'
env \
  CUDA_DEVICE_ORDER=PCI_BUS_ID \
  CUDA_VISIBLE_DEVICES="${QC_GPU}" \
  "${PYTHON_BIN}" experiments/asre_diagnosis/round2/validate_state_bank.py \
...
summary_path,metadata_path,per_sample_path,manifest_path,condition,enabled_json,expected,checkpoint,stats,repo=sys.argv[1:]
try:
...
sys.exit(0 if ok else 1)' \
    ...
    "${EXPECTED_SAMPLES}" "${CHECKPOINT}" "${DATASET_STATS}" "${REPO_ROOT}"
}

declare -a needs_launch=()

# Complete the safety preflight for all eight directories before starting any
...
for condition_index in "${!CONDITIONS[@]}"; do
  ...
  if condition_complete "${condition}" "${enabled_schedule}"; then
...
for condition_index in "${!CONDITIONS[@]}"; do
  ...
  env \
    ...
    "${PYTHON_BIN}" experiments/asre_diagnosis/round2/replay_state_bank.py \
      task=libero_uncond_2cam224_1e-4 \
      ckpt="${CHECKPOINT}" \
      model.load_text_encoder=false \
      EVALUATION.device=cuda:0 \
      ...
      ASRE_DIAGNOSIS.protocol=round2_keep_schedules \
...
if (( failed != 0 )); then
  printf 'At least one Round-2 offline replay failed.\n' >&2
  exit 1
fi
for condition_index in "${!CONDITIONS[@]}"; do
  condition="${CONDITIONS[$condition_index]}"
  if ! condition_complete "${condition}" "${ENABLED_SCHEDULES[$condition_index]}"; then
    printf 'Missing/incomplete Round-2 offline condition: %s\n' "${condition}" >&2
    exit 1
  fi
done
trap - EXIT INT TERM
printf 'Round-2 QC and all eight offline replays completed successfully.\n'
