"""Replay one ASRE Round 3A keep schedule over the immutable valid state set."""
...
def _require_clean_worktree() -> None:
    ...
    if output.strip():
        raise RuntimeError(
            "ASRE Round 3A offline replay requires a clean Git worktree. Commit "
            "the Round-3A implementation first; no changes will be committed or "
...
def replay_state_bank(cfg: DictConfig) -> None:
    ...
    if not bool(cfg.ASRE_DIAGNOSIS.get("enabled", False)):
        raise ValueError("Round-3A offline replay requires ASRE_DIAGNOSIS.enabled=true.")
    if str(cfg.ASRE_DIAGNOSIS.get("protocol", "")) != ROUND3A_PROTOCOL:
        raise ValueError(
            f"Round-3A offline replay requires ASRE_DIAGNOSIS.protocol={ROUND3A_PROTOCOL}."
        )
    ...
    trusted_parent_digest = os.environ.get(
        "ASRE_ROUND3A_TRUSTED_PREFLIGHT_MANIFEST_SHA256"
    )
    validate_existing_artifacts(
        ...
        # A direct replay hashes the checkpoint itself. The parent shell may
        # skip the redundant 12-GB read only after its strict preflight has
        # exported the digest of this exact valid manifest.
    ...
    if not selected_records:
        raise ValueError("The Round-3A valid state-bank manifest contains no samples.")
    ...
    if executed_prefix_length != 10 or executed_prefix_length != int(
        ...
        raise ValueError(
            "ASRE Round 3A executed_prefix_length must equal the pre-registered "
    ...
    if source_mismatches:
        raise ValueError(
            "Round-3A replay configuration is incompatible with the source state bank: "
    ...
    condition_index = build_round3a_conditions(num_layers).index(condition)
    cuda_visible_devices = os.environ.get("CUDA_VISIBLE_DEVICES")
    if cuda_visible_devices != str(condition_index):
        raise ValueError(
            "Round-3A offline GPU mapping must be condition 0/1/2 -> physical "
            f"GPU 0/1/2; condition={condition.name}, expected "
            f"CUDA_VISIBLE_DEVICES={condition_index}, got {cuda_visible_devices!r}."
        )
    if str(model_device) != "cuda:0":
        raise ValueError(
            f"Round-3A offline replay requires logical model device cuda:0, got {model_device}."
        )
    ...
    if output_dir.exists() and any(output_dir.iterdir()):
        raise FileExistsError(
            f"Refusing to overwrite completed or partial Round-3A replay output: {output_dir}"
        )
    ...
    metadata.update(
        {
            "artifact_type": "asre_round3a_offline_state_bank_replay",
            "status": "running",
    ...
    print(f"Round-3A offline replay complete: {output_dir}")
