"""Launch four isolated Salvage-A calibration or held-out layer shards."""

from __future__ import annotations

import argparse
import json
import os
import shlex
import signal
import subprocess
import sys
import time
from pathlib import Path
from typing import Any


PROJECT_ROOT = Path(__file__).resolve().parents[3]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from experiments.asre_diagnosis.common import (  # noqa: E402
    SALVAGE_A_PROTOCOL,
    atomic_write_json,
    git_commit,
    now_iso,
    sha256_file,
    sha256_json,
)
from experiments.asre_diagnosis.g0.launch_four_gpu import (  # noqa: E402
    _child_environment,
    _launcher_lock,
)
from experiments.asre_diagnosis.round4a.launch_wave import (  # noqa: E402
    _gpu_inventory,
    stable_gpu_inventory,
)
from experiments.asre_diagnosis.salvage_a.action_sensitivity import (  # noqa: E402
    frozen_probe_records,
)
from experiments.asre_diagnosis.salvage_a.basis import LATE_LAYERS  # noqa: E402


def _read(path: Path) -> dict[str, Any]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise TypeError(f"Expected JSON object: {path}")
    return payload


def _valid_report(
    path: Path,
    *,
    phase: str,
    worker_index: int,
    identity: dict[str, Any],
) -> bool:
    if not path.is_file():
        return False
    try:
        report = _read(path)
    except (OSError, ValueError, TypeError, json.JSONDecodeError):
        return False
    metadata_valid = all(
        (
            report.get("status") == "complete",
            report.get("protocol") == SALVAGE_A_PROTOCOL,
            report.get("phase") == phase,
            report.get("worker_index") == worker_index,
            report.get("layers") == list(LATE_LAYERS[worker_index::4]),
            report.get("git_commit_hash") == identity["git_commit_hash"],
            report.get("split_sha256") == identity["split_sha256"],
            report.get("state_selection_sha256")
            == identity["state_selection_sha256"],
            report.get("checkpoint_sha256") == identity["checkpoint_sha256"],
            report.get("donor_mapping_sha256")
            == identity["donor_mapping_sha256"],
            report.get("donor_manifest_sha256")
            == identity["donor_manifest_sha256"],
            report.get("differentiable_path_report_sha256")
            == identity["differentiable_path_report_sha256"],
        )
    )
    if not metadata_valid:
        return False
    expected_matrices = {
        f"layer{layer:02d}_{kind}"
        for layer in LATE_LAYERS[worker_index::4]
        for kind in ("k", "v")
    }
    artifacts = report.get("artifacts")
    if not isinstance(artifacts, dict) or set(artifacts) != expected_matrices:
        return False
    for record in artifacts.values():
        if not isinstance(record, dict):
            return False
        artifact = Path(str(record.get("path", ""))).expanduser().resolve()
        expected_sha = record.get("sha256")
        if (
            not isinstance(expected_sha, str)
            or len(expected_sha) != 64
            or not artifact.is_file()
            or sha256_file(artifact) != expected_sha
        ):
            return False
    return True


def launch(args: argparse.Namespace) -> Path:
    gpu_ids = tuple(int(value) for value in args.gpu_ids)
    if len(gpu_ids) != 4 or len(set(gpu_ids)) != 4 or any(value < 0 for value in gpu_ids):
        raise ValueError("Salvage-A fitting requires four distinct nonnegative GPU IDs.")
    inventory = _gpu_inventory()
    available = {int(record["index"]) for record in inventory}
    if set(gpu_ids) - available:
        raise ValueError(f"Unavailable GPUs: {sorted(set(gpu_ids) - available)}")
    output_dir = args.output_dir.resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    log_dir = output_dir.parent / "logs" / f"{args.phase}_shards"
    log_dir.mkdir(parents=True, exist_ok=True)
    differentiable_path = args.differentiable_path_report.resolve()
    differentiable = _read(differentiable_path)
    expected_differentiable_links = {
        "git_commit_hash": git_commit(PROJECT_ROOT),
        "split_sha256": sha256_file(args.split.resolve()),
        "state_selection_sha256": sha256_file(args.state_selection.resolve()),
        "donor_mapping_sha256": sha256_file(args.donor_mapping.resolve()),
        "donor_manifest_sha256": sha256_file(args.donor_manifest.resolve()),
        "checkpoint_sha256": sha256_file(args.checkpoint.resolve()),
    }
    if (
        differentiable.get("protocol") != SALVAGE_A_PROTOCOL
        or differentiable.get("passed") is not True
        or any(
            differentiable.get(key) != value
            for key, value in expected_differentiable_links.items()
        )
    ):
        raise ValueError(
            "Salvage-A shards require the matching passed differentiable-path gate."
        )
    identity = {
        "artifact_type": "asre_salvage_a_shard_launcher_config",
        "schema_version": 1,
        "protocol": SALVAGE_A_PROTOCOL,
        "phase": args.phase,
        "git_commit_hash": git_commit(PROJECT_ROOT),
        "gpu_ids": list(gpu_ids),
        "gpu_inventory": stable_gpu_inventory(
            [record for record in inventory if int(record["index"]) in gpu_ids]
        ),
        "split_path": str(args.split.resolve()),
        "split_sha256": sha256_file(args.split.resolve()),
        "state_selection_path": str(args.state_selection.resolve()),
        "state_selection_sha256": sha256_file(args.state_selection.resolve()),
        "checkpoint_path": str(args.checkpoint.resolve()),
        "checkpoint_sha256": sha256_file(args.checkpoint.resolve()),
        "donor_mapping_path": str(args.donor_mapping.resolve()),
        "donor_mapping_sha256": sha256_file(args.donor_mapping.resolve()),
        "donor_manifest_path": str(args.donor_manifest.resolve()),
        "donor_manifest_sha256": sha256_file(args.donor_manifest.resolve()),
        "donor_root": str(args.donor_root.resolve()),
        "differentiable_path_report_path": str(differentiable_path),
        "differentiable_path_report_sha256": sha256_file(differentiable_path),
        "probe_records": frozen_probe_records() if args.phase == "fit" else [],
        "no_ddp": True,
    }
    identity["identity_sha256"] = sha256_json(identity)
    config_path = output_dir / f"{args.phase}_launcher_config.json"
    if config_path.is_file() and _read(config_path) != identity:
        raise RuntimeError(f"Refusing incompatible Salvage-A resume in {output_dir}.")
    if not config_path.exists():
        atomic_write_json(config_path, identity)

    children: list[tuple[int, subprocess.Popen[Any], Any, Path]] = []
    failure: str | None = None
    start = now_iso()
    with _launcher_lock(output_dir / args.phase) as lock_fd:
        try:
            for worker_index, physical_gpu in enumerate(gpu_ids):
                report_path = output_dir / f"{args.phase}_shard{worker_index}.json"
                if _valid_report(
                    report_path,
                    phase=args.phase,
                    worker_index=worker_index,
                    identity=identity,
                ):
                    continue
                attempt = 1
                log_path = log_dir / f"shard{worker_index}.attempt{attempt:02d}.log"
                while log_path.exists():
                    attempt += 1
                    log_path = log_dir / f"shard{worker_index}.attempt{attempt:02d}.log"
                command = [
                    str(args.python.resolve()),
                    "-m",
                    "experiments.asre_diagnosis.salvage_a.fit_worker",
                    "--phase",
                    args.phase,
                    "--worker-index",
                    str(worker_index),
                    "--checkpoint",
                    str(args.checkpoint.resolve()),
                    "--split",
                    str(args.split.resolve()),
                    "--state-selection",
                    str(args.state_selection.resolve()),
                    "--donor-mapping",
                    str(args.donor_mapping.resolve()),
                    "--donor-manifest",
                    str(args.donor_manifest.resolve()),
                    "--donor-root",
                    str(args.donor_root.resolve()),
                    "--differentiable-path-report",
                    str(differentiable_path),
                    "--output-dir",
                    str(output_dir),
                ]
                handle = log_path.open("x", encoding="utf-8")
                handle.write(shlex.join(command) + "\n")
                handle.flush()
                environment = _child_environment(physical_gpu)
                environment["ASRE_SALVAGE_A_PHYSICAL_GPU"] = str(physical_gpu)
                process = subprocess.Popen(
                    command,
                    cwd=PROJECT_ROOT,
                    env=environment,
                    stdout=handle,
                    stderr=subprocess.STDOUT,
                    start_new_session=True,
                    pass_fds=(lock_fd,),
                )
                children.append((worker_index, process, handle, log_path))
                if args.launch_stagger_seconds > 0 and worker_index < 3:
                    time.sleep(args.launch_stagger_seconds)
            pending = list(children)
            while pending:
                for child in list(pending):
                    worker_index, process, handle, log_path = child
                    returncode = process.poll()
                    if returncode is None:
                        continue
                    pending.remove(child)
                    handle.close()
                    if returncode != 0:
                        failure = (
                            f"Salvage-A {args.phase} shard {worker_index} exited "
                            f"{returncode}; inspect {log_path}."
                        )
                        for _, other, other_handle, _ in pending:
                            if other.poll() is None:
                                os.killpg(other.pid, signal.SIGTERM)
                                other.wait(timeout=30)
                            other_handle.close()
                        pending.clear()
                        break
                if pending:
                    time.sleep(2)
        except KeyboardInterrupt:
            for _, process, handle, _ in children:
                if process.poll() is None:
                    os.killpg(process.pid, signal.SIGTERM)
                handle.close()
            raise

    all_complete = all(
        _valid_report(
            output_dir / f"{args.phase}_shard{worker_index}.json",
            phase=args.phase,
            worker_index=worker_index,
            identity=identity,
        )
        for worker_index in range(4)
    )
    summary_path = output_dir / f"{args.phase}_launcher_summary.json"
    atomic_write_json(
        summary_path,
        {
            "artifact_type": "asre_salvage_a_shard_launcher_summary",
            "schema_version": 1,
            "protocol": SALVAGE_A_PROTOCOL,
            "phase": args.phase,
            "status": "complete" if all_complete and failure is None else "failed",
            "all_succeeded": all_complete and failure is None,
            "failure": failure,
            "start_timestamp": start,
            "end_timestamp": now_iso(),
            "identity": identity,
            "no_ddp": True,
        },
    )
    if failure is not None or not all_complete:
        raise RuntimeError(failure or f"Salvage-A {args.phase} shards incomplete.")
    return summary_path


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--phase", choices=("fit", "heldout"), required=True)
    parser.add_argument("--python", type=Path, default=Path(sys.executable))
    parser.add_argument("--gpu-ids", nargs=4, type=int, required=True)
    parser.add_argument("--checkpoint", type=Path, required=True)
    parser.add_argument("--split", type=Path, required=True)
    parser.add_argument("--state-selection", type=Path, required=True)
    parser.add_argument("--donor-mapping", type=Path, required=True)
    parser.add_argument("--donor-manifest", type=Path, required=True)
    parser.add_argument("--donor-root", type=Path, required=True)
    parser.add_argument("--differentiable-path-report", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--launch-stagger-seconds", type=float, default=70.0)
    output = launch(parser.parse_args())
    print(f"Salvage-A shard wave complete: {output}")


if __name__ == "__main__":
    main()
