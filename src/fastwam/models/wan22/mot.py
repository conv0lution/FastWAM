...
class MoT(nn.Module):
    def __init__(
        ...
        checkpointed_experts = [
            ...
            if bool(getattr(self.mixtures[name], "use_gradient_checkpointing", False))
        ]
        if checkpointed_experts:
            raise ValueError(
                "Wan expert gradient checkpointing is not supported by the compiled tensor core: "
